...
def run_training(
     run_title: str, config: dict, eval_only: bool, epochs: int = 100, data_to_use: float = 0.1, Lambda = [0.0,0.0], verbose: bool = False
) -> None:
    global INPUT_SIZE

    draw = Draw(output_dir=f"{run_title}/plots")
    ...
    if not eval_only:
        teacher = TeacherAutoencoder((18, 14, 1), Lambda).get_model()
        ...
        teachers_scn=[TeacherScnAutoencoder((6, 14, 1), Lambda).get_model(name=f"teacher_scn_{i+1}") for i in range(3)]
        ...
        teacher_spr = TeacherScnAutoencoder((6, 14, 1), Lambda).get_model(name="teacher_spr")
    ...
    # Training results
    if not os.path.exists(f"{run_title}/plots"): os.makedirs(f"{run_title}/plots")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        help="Lambdas to use for L2 regularization, [0] for kernel, [1] for activity",
        default=0.0,
    )
    parser.add_argument(
        "-verbose",
...
def main(args_in=None) -> None:
    ...
    run_training(run_title=args.run_title, config=config, eval_only=args.evaluate_only, epochs=args.epochs, data_to_use=args.data_to_use, Lambda=args.Lambda, verbose=args.verbose)
