...
def run_training(
    ...
    for i in range(len(datasets_signal)):
        ...
        X_test.append(X_tmp)
    X_test_scn = X_test.copy()

    for dataset in [X_train, X_val, X_spr_train, X_spr_val]:
        for i in range(dataset.shape[0]):
            sum = np.sum(dataset[i])
            if (sum>0): dataset[i] = dataset[i] / sum
    for dataset in [X_scn_train, X_scn_val]:
        for i in range(dataset.shape[0]):
            for j in range(3):
                sum = np.sum(dataset[i,j])
                if (sum>0): dataset[i,j] = dataset[i,j] / sum
    for dataset in X_test:
        for i in range(dataset.shape[0]):
            sum = np.sum(dataset[i])
            if (sum>0): dataset[i] = dataset[i] / sum
    for dataset in X_test_scn:
        for i in range(dataset.shape[0]):
            for j in range(3):
                sum = np.sum(dataset[i,j*6:j*6+6])
                if (sum>0): dataset[i,j*6:j*6+6] = dataset[i,j*6:j*6+6] / sum
    ...
    if not eval_only:
        ...
        t_spr_mc = ModelCheckpoint(f"runs/{run_title}/models/teacher_spr", save_best_only=True)
        t_spr_log = CSVLogger(f"runs/{run_title}/models/teacher_spr/training.log", append=True)
    ...
    draw.plot_loss_history(log["loss"], log["val_loss"], "teacher_training_history", ylim=[0.0, 0.0003])
    log = []
    for i in range(3):
        log.append(pd.read_csv(f"runs/{run_title}/models/teacher_scn_{i+1}/training.log"))
        draw.plot_loss_history(log[i]["loss"], log[i]["val_loss"], f"teacher_scn_{i+1}_training_history", ylim=[0.0, 0.0003])
    draw.plot_multiple_loss_history([[log[0]["loss"], log[0]["val_loss"], f"teacher_scn_0"],
                                     ...
                                     "teacher_scn_training_history_overlay", ylim=[0.0, 0.0003])
    log = pd.read_csv(f"runs/{run_title}/models/teacher_spr/training.log")
    draw.plot_loss_history(log["loss"], log["val_loss"], "teacher_spr_training_history", ylim=[0.0, 0.0003])
    ...
    X_all_test = np.concatenate((X_test_scn), axis=0)
    ...
    for i in range(10):
        draw.plot_reconstruction_results(np.array([X_test[0][i]]), np.array([y_pred[0][0][i]]), loss=y_loss[0][0][i], name=f"comparison_background_{model_names_short[0]}_{i}")
        draw.plot_reconstruction_results(np.array([X_test_scn[0][i]]), np.array([y_pred[1][0][i]]), loss=y_loss[1][0][i], name=f"comparison_background_{model_names_short[1]}_{i}")
        draw.plot_reconstruction_results(np.array([X_test_scn[0][i]]), np.array([y_pred[2][0][i]]), loss=y_loss[2][0][i], name=f"comparison_background_{model_names_short[2]}_{i}")

    print("Finished plotting reconstruction examples... plotting roc curves")
    # Declaring lists to be used in roc curve
    y_true, inputs, inputs_scn, y_pred_roc = [], [], [], [[], [], []]
    y_pred_roc_sig = [[], [], [], [], [], []]
    ...
    for i in range(1, len(signal_names)):
        inputs_scn.append(np.concatenate((X_test_scn[i], X_test_scn[0])))

    draw.plot_roc_curve(y_true, y_pred_roc[0], signal_names, inputs, f"teacher_{model_names_short[0]}")
    draw.plot_roc_curve(y_true, y_pred_roc[1], signal_names, inputs_scn, f"teacher_{model_names_short[1]}")
    draw.plot_roc_curve(y_true, y_pred_roc[2], signal_names, inputs_scn, f"teacher_{model_names_short[2]}")
    for i in range(len(signal_names)-1):
        draw.plot_roc_curve([y_true[i], y_true[i], y_true[i]], y_pred_roc_sig[i+1], model_names_long, [inputs[i], inputs_scn[i], inputs_scn[i]], f"teacher_{signal_names[i]}")
    ...
    for i in range(3):
        ...
        draw.plot_anomaly_score_distribution(list_results[i], [*results[i]], f"anomaly_score_teacher_{model_names_short[i]}", xlim=[0.0,0.001])
    for i in range(len(signal_names)):
        for j in range(3):
            results_sig[i][f'{model_names_long[j]}'] = y_loss[j][i]
        list_results_sig.append(list(results_sig[i].values()))
        draw.plot_anomaly_score_distribution(list_results_sig[i], [*results_sig[i]], f"anomaly_score_signal_{signal_names[i]}", xlim=[0.0,0.001])
    print("Finished plotting anomaly score distribution... plotting scatter plots and finding score correlations")
    # Score scatter plot
    scatter_fit = [[], []]
    for i in range(len(signal_names)):
        for j in range(2):
            scatter_fit[j].append(draw.plot_scatter_score_comparison(np.sqrt(y_loss[0][i]), np.sqrt(y_loss[j+1][i]), f"{model_names_short[0]}", f"{model_names_short[j+1]}", f"{model_names_short[0]}_{model_names_short[j+1]}_{signal_names[i]}", limits="equal"))
